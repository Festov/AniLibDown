...
Синхронизируются только **статусы списка** (без учёта отдельных серий). Привязки тайтлов сохраняются **локально на телефоне** и сбрасываются при переустановке приложения.
