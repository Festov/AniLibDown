...
- **Уведомления** — о выходе новых серий и о завершении загрузки
- **Аккаунт AniLiberty** — обязательный вход, профиль, коллекции («Смотрю», «В планах», «Просмотрено» и др.) с синхронизацией
- **Shikimori** (опционально) — привязка тайтлов, статусы списка, синхронизация серии при досмотре, экспорт/импорт привязок
- **Настройки** — тема, заставка, качество по умолчанию, параметры плеера и загрузок (в т.ч. только Wi‑Fi), видимость категорий коллекции, очистка кэша и другое
